...
if length(filterFreqs) == 1
    
    [~, d] = wfEllipFilter_HighPass(traces(1),filterFreqs);
    
elseif length(filterFreqs) == 2
    
    [~, d] = wfEllipFilter_band(traces(1),filterFreqs);
...
for k = 1:length(traces)
    ...
    if length(filterFreqs) == 1
        ...
        trF     = wfEllipFilter_HighPass(tr,filterFreqs, 'zero', d);
        tr      = wfEllipFilter_LowPass(tr,filterFreqs, 'zero', d);
        
    elseif length(filterFreqs) == 2
       
        trF     = wfEllipFilter_band(tr,filterFreqs, 'zero', d);
